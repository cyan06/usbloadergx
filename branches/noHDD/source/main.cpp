...
//	sleep(5);
	...
	if (argc > 1 && argv[1])
	{
		if (strlen(argv[1])==6)
			...
			strncpy(headlessID, argv[1], sizeof(headlessID));
                        InitCheckThread();
                        while(checkthreadState!=1)usleep(5000);
                        mountMethod = 0;
                        checkthreadState=-1;
                        ExitCheckThread();
                        MainMenu(MENU_EXIT,headlessID);
                }
	}
    ...
    //wait for the check thread to run through at least 1 time
    while(checkthreadState==-2)usleep(5000);
